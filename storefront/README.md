...
- The root directory is represented by the `[countryCode]` folder. This indicates a dynamic route based on the country code. The this will be populated by the countries you set up in your server. The param is then used to fetch region specific prices, languages, etc.
...
The lib directory contains all utilities like the JS client functions, util functions, config and constants. 
...
The middleware also sets a cookie based on the onboarding status of a user. This is related to the Admin onboarding flow, and may be safely removed in your production storefront.
